...
const ManageAllOrders = () => {
  ...
  useEffect(() => {
    fetch("https://fierce-taiga-81798.herokuapp.com/allOrder")
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    ...
    if (sure) {
      fetch(`https://fierce-taiga-81798.herokuapp.com/allOrder/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const onSubmit = (data) => {
    fetch(`https://fierce-taiga-81798.herokuapp.com/status/${orderId}`, {
      method: "PUT",
      headers: {
